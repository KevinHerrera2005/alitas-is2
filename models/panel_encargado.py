...
def _panel_encargado_log_request():
    ...
    try:
        logger_.Logger.add_to_log(
            "info", f"{request.method} {request.path}", "panel_encargado_request", fecha
        )
    except Exception:
        pass


def _solo_error_response():
    return Response(
...
def _correo_de_proveedor(proveedor: Proveedor | None) -> str | None:
    if proveedor is None:
        return None
    return (
        getattr(proveedor, "email", None) or getattr(proveedor, "Email", None) or ""
    ).strip() or None
...
def _unidad_nombre_de_insumo(ins):
    ...
    if unidad_obj is not None:
        unidad_nombre = (
            getattr(unidad_obj, "Nombre", None)
            or getattr(unidad_obj, "Nombre_Unidad", None)
            or unidad_nombre
        )
...
def _proveedores_de_insumos(insumos_ids):
    ...
    for prov in rows:
        ...
        out.append(
            {
                "id": pid,
                "nombre": getattr(prov, "Nombre_Proveedor", None)
                or getattr(prov, "Nombre", None)
                or str(pid),
...
def _insumos_ui_unicos_por_sucursal(sucursal_id: int):
    ...
    insumos = (
        Insumo.query.filter(Insumo.ID_sucursal == sucursal_id)
        ...
        salida.append(
            {
                ...
                "nombre": (
                    g["nombre"] or getattr(rep, "Nombre_insumo", "") or ""
                ).strip(),
...
def enviar_correo_orden(proveedor: Proveedor, por_sucursal: dict):
    ...
    prov_nombre = (
        getattr(proveedor, "Nombre_Proveedor", None)
    ...
    data_ordenada = sorted(
        ...
        key=lambda d: _norm(
            getattr((d.get("sucursal") or None), "Descripcion", "") or ""
        ),
    )
    ...
    for data in data_ordenada:
        ...
        for insumo, cantidad, unidad_nombre in items:
            nombre_ins = (
                getattr(insumo, "Nombre_insumo", None)
                or getattr(insumo, "nombre", None)
                or ""
            )
    ...
    try:
        ...
        fecha = datetime.now().strftime("%Y%m%d-%H%M%S")
        logger_.Logger.add_to_log(
            "error", str(error), "encargar_insumos_enviar_correo", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "encargar_insumos_enviar_correo", fecha
        )
        return False, "Gmail rechazó la autenticación (usa App Password)"
    except Exception as e:
        fecha = datetime.now().strftime("%Y%m%d-%H%M%S")
        logger_.Logger.add_to_log(
            "error", str(e), "encargar_insumos_enviar_correo", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "encargar_insumos_enviar_correo", fecha
        )
...
def encargar_insumos_data():
    ...
    try:
        ...
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_encargar_insumos_data", fecha
        )
        logger_.Logger.add_to_log(
            "error",
            traceback.format_exc(),
            "panel_encargado_encargar_insumos_data",
            fecha,
        )
        return (
            jsonify({"ok": False, "insumos": [], "error": "Error al cargar insumos."}),
            200,
        )
...
def encargar_insumos():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if not lineas_raw:
                flash(
                    "Debes guardar al menos un insumo con su proveedor y cantidad.",
                    "warning",
                )
            ...
            for linea in lineas_data:
                ...
                if getattr(insumo, "ID_sucursal", None) != sucursal_id:
                    flash(
                        "Hay insumos que no pertenecen a la sucursal indicada.",
                        "danger",
                    )
                    params = {"todos": "1"} if modo_todos else {}
                    return redirect(
                        url_for("panel_encargado.encargar_insumos", **params)
                    )

                unidad_nombre = (
                    linea.get("unidad") or _unidad_nombre_de_insumo(insumo) or "unidad"
                )

                if proveedor_id not in agrupado:
                    agrupado[proveedor_id] = {
                        "proveedor": proveedor,
                        "por_sucursal": {},
                    }
                ...
                if key_ins in items_map:
                    ...
                    items_map[key_ins] = (
                        ins_prev,
                        float(cant_prev) + float(cantidad),
                        uni_prev or unidad_nombre,
                    )
                else:
            ...
            proveedores_sin_email = [
                ...
                if not _correo_de_proveedor(d["proveedor"])
            ]
            if proveedores_sin_email:
                nombres = ", ".join(
                    getattr(p, "Nombre_Proveedor", None)
                    or getattr(p, "Nombre", None)
                    or ""
                    for p in proveedores_sin_email
                )
                flash(
                    f"Los siguientes proveedores no tienen definido un email: {nombres}",
                    "danger",
                )
                params = {"todos": "1"} if modo_todos else {}
                return redirect(url_for("panel_encargado.encargar_insumos", **params))
            ...
            if (
                id_empleado_encargado is None
                or Empleado.query.get(id_empleado_encargado) is None
            ):
                flash(
                    "No se pudo determinar el empleado encargado para la orden.",
                    "danger",
                )
                params = {"todos": "1"} if modo_todos else {}
                return redirect(url_for("panel_encargado.encargar_insumos", **params))
            ...
            for datos in agrupado.values():
                ...
                for data_suc in por_sucursal.values():
                    ...
                    items.sort(
                        key=lambda t: _norm(getattr(t[0], "Nombre_insumo", "") or "")
                    )
                    ...
                    for insumo, cantidad, _ in items:
                        id_unidad = getattr(insumo, "ID_Unidad", None) or getattr(
                            insumo, "ID_Unidad_medida", None
                        )
                        if id_unidad is None:
                            db.session.rollback()
                            flash(
                                f"El insumo '{getattr(insumo, 'Nombre_insumo', '')}' no tiene unidad configurada.",
                                "danger",
                            )
                            params = {"todos": "1"} if modo_todos else {}
                            return redirect(
            ...
            for datos in agrupado.values():
                ...
                if not ok:
                    flash(
                        f"Correo NO enviado a {getattr(proveedor, 'Nombre_Proveedor', '')}: {err}",
                        "danger",
                    )
        ...
        sucursales_q = (
            Sucursal.query.filter(Sucursal.estado == 1)
        ...
        for s in sucursales_q:
            ...
            sucursales_ui.append(
                {"id": int(s.ID_sucursal), "nombre": s.Descripcion, "direccion": d}
            )
        ...
        insumos_ui = (
            _insumos_ui_unicos_por_sucursal(int(sucursal_id))
        ...
        try:
            ...
            pass
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_encargar_insumos", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "panel_encargado_encargar_insumos", fecha
        )
...
def ordenes_proveedores_encargado():
    ...
    try:
        logger_.Logger.add_to_log(
            "info",
            "click_ordenes_proveedores_encargado",
            "panel_encargado_ordenes_proveedores_encargado",
            fecha,
        )
        ...
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_ordenes_template_missing", fecha
        )
        logger_.Logger.add_to_log(
            "error",
            traceback.format_exc(),
            "panel_encargado_ordenes_template_missing",
            fecha,
        )
        return _solo_error_response()
        ...
        try:
            ...
            pass
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_ordenes_db", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "panel_encargado_ordenes_db", fecha
        )
        return _solo_error_response()
        ...
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "panel_encargado_ordenes", fecha
        )
        return _solo_error_response()
...
def historial_ordenes_proveedores_encargado():
    ...
    try:
        logger_.Logger.add_to_log(
            "info",
            "click_historial_ordenes_proveedores_encargado",
            "panel_encargado_historial_ordenes_proveedores_encargado",
            fecha,
        )
        ...
        ordenes = (
            q.order_by(OrdenesProveedores.ID_Orden_Proveedor.desc()).limit(300).all()
        )

        return render_template(
            "historial_ordenes_proveedores_encargado.html", ordenes=ordenes
        )

    except TemplateNotFound as error:
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_historial_template_missing", fecha
        )
        logger_.Logger.add_to_log(
            "error",
            traceback.format_exc(),
            "panel_encargado_historial_template_missing",
            fecha,
        )
        return _solo_error_response()
        ...
        try:
            ...
            pass
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_historial_db", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "panel_encargado_historial_db", fecha
        )
        return _solo_error_response()
        ...
        try:
            ...
            pass
        logger_.Logger.add_to_log(
            "error", str(error), "panel_encargado_historial", fecha
        )
        logger_.Logger.add_to_log(
            "error", traceback.format_exc(), "panel_encargado_historial", fecha
        )
        return _solo_error_response()
