...
def mis_pedidos():
    try:
        ...
        logger_.Logger.add_to_log("error", str(error), "mis_pedidos", fecha)
        logger_.Logger.add_to_log("error", traceback.format_exc(), "mis_pedidos", fecha)
...
def imprimir_factura(numero_factura):
    try:
        ...
        fecha = datetime.now().strftime("%Y%m%d-%H%M%S")
        logger_.Logger.add_to_log("error", str(error), "imprimir_factura", fecha)
        logger_.Logger.add_to_log("error", traceback.format_exc(), "imprimir_factura", fecha)
        return "esto es un error", 501
